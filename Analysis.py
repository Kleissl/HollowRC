...
def planeSection(section, SF, Mat):
    ...
    H, H_yield, xopt = OptimumWallShear(section, SF, Mat, dist)
    # H, H_yield, xopt = OptimumNodeShear(section, SF, Mat, dist, x, opt)
...
# Shear optimizing based on wall shear forces
def OptimumWallShear(section, SF, Mat, dist):

    # --------------- Dual-section analysis for better initial guess ---------------
    _, H = dualSection(section, SF, Mat)  #                                                      <-----  CONTAINS AN UNNESESSARY BENDING OPTIMIZATION !
    V_wall = []
    ...
    # x0 = np.array(V_yield)
    # x0 = 0.5*x0             # 0.5 gives a more reliable optimization than 0.0 or 1.0
    x0 = [Vi_wall if abs(Vi_wall) <= V_yield[i] else math.copysign(V_yield[i], Vi_wall) for i,Vi_wall in enumerate(V_wall)] # note: copysign(x,y) return x with the sign of y
    print('x0:', x0)
